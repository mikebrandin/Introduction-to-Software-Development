...
public interface IGameBoard {
    ...
    public default boolean checkIfFree(int c){
        ...
        for (int i = 0; i < getNumRows(); i++){
            ...
            if (isPlayerAtPos(temp, 'X') || isPlayerAtPos(temp, 'O')){//use function calls to check if player is at position
                tokenCount++;
            }
        }
    ...
    public default boolean checkHorizWin(BoardPosition pos, char p){
        ...
        for (int i = 0; i < getNumColumns(); i++){
            ...
            else if (successiveCounter >= getNumToWin()){//if counter reaches or exceeds the number to win return true
                return true;
            }
